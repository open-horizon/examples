  ...
  render() {
    ...
    if (info.length > 1) {
      renderList = <div>
        ...
        <div>{info.map(o => <Tag key={o.edgenode} type="custom"><a href={`/edge-nodes/details?id=${o.edgenode}`}>{o.edgenode}</a><br /></Tag>)}</div>  
      </div>
    }
    ...
    return (
      <div>
        ...
        {info.length === 1 &&
          <div>
            <h1><a href={`/edge-nodes/details?id=${info[0].edgenode}`}>{info[0].edgenode}</a></h1>
            <p>
