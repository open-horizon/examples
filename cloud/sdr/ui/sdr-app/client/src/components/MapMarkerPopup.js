  ...
  render() {
    ...
    if (info.length > 1) {
      renderList = <div>
        ...
        <div>{info.map(o => <Tag key={o.edgenode} type="custom"><a href={`/app/edge-nodes/${o.edgenode}`}>{o.edgenode}</a><br /></Tag>)}</div>  
      </div>
    }
    ...
    return (
      <div>
        ...
        {info.length === 1 &&
          <div>
            <h1><a href={`/app/edge-nodes/${info[0].edgenode}`}>{info[0].edgenode}</a></h1>
            <p>
