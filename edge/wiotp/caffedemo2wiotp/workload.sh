...
# This workload expects the PWSMS microservice to be running.  Run 'make' in the
# sibling directory "../service" to start that microservice running.  You
...
# MOCK: if "true", just pretend to call the pwsms REST API
...
while true; do
  ...
  if [[ "$MOCK" == "true" ]]; then
    output='{"r": {"confidence": 0.95, "class": "coffee_cup"}} 200'
    curlrc=0
  else
    output=$(curl -sS -w %{http_code} "http://caffedemo:8367/v1/caffedemo")
  ...
  if [[ "$curlrc" != 0 ]]; then
    ...
    # Send a "object" event to the Watson IoT Platform containing the data
    clientId="$CLASS_ID:$HZN_ORGANIZATION:$DEVICE_TYPE:$DEVICE_ID"
    if [[ "$CLASS_ID" == "g" ]]; then
      topic="iot-2/type/$DEVICE_TYPE/id/$DEVICE_ID/evt/obj/fmt/json"
    else
